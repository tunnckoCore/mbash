 ...
 */
	...
	function tds4($data, $salt = 1, $cryptLevel = 5, $crypterVersion = 4, $cycleRounds = null) {
		...
		// if $salt set to 1, we use MB_HASH_SECURITY_KEY in hashing process..
		if($salt == 1) {
			if(defined('MB_HASH_SECURITY_KEY')) {
				$salton = MB_HASH_SECURITY_KEY;
				/* Security level 1 */
		...
		// if $salt set to 2, we use DEFAULT STRING in hashing process..
		} elseif($salt == 2) {
